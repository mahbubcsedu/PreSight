...
double _lengthOfBBCenterInterceptInObject;
double _lengthOfSTCenterInterceptInObject;
double _lengthOfOrgBBCenterInterceptInObject;
...
void writeOutput() {
	...
	header.append("\tlengthOfBBCenterInterceptInObject");
	header.append("\tlengthOfSTCenterInterceptInObject");
	header.append("\tlengthOfOrgBBCenterInterceptInObject");
	...
	sstream.str(""); sstream.clear();
	sstream << _lengthOfBBCenterInterceptInObject;
	result.append("\t\"").append(sstream.str()).append("\"");
	
	sstream.str(""); sstream.clear();
	sstream << _lengthOfSTCenterInterceptInObject;
	result.append("\t\"").append(sstream.str()).append("\"");
		
	sstream.str(""); sstream.clear();
	sstream << _lengthOfOrgBBCenterInterceptInObject;
	result.append("\t\"").append(sstream.str()).append("\"");
...
void executeCore() {
	...
	_lengthOfOrgBBCenterInterceptInObject=lengthBBCenterInterceptInObject;
	...
	centerofBBInObject = tranformPoint(H_invt, centerOfBBInImage);
	bottomInterceptOfBBCenterInObject = tranformPoint(H_invt, bottomInterceptOfBBCenterInImage);

	CvPoint2D32f bottomInterceptOfSTCenterInObject = tranformPoint(H_invt, bottomInterceptOfSTCenterInImage);

	 delta_x = bottomInterceptOfBBCenterInObject.x - centerofBBInObject.x;
	 delta_Y = bottomInterceptOfBBCenterInObject.y - centerofBBInObject.y;
	lengthBBCenterInterceptInObject = sqrt(((delta_x*delta_x)+(delta_Y*delta_Y)));
	_lengthOfBBCenterInterceptInObject=lengthBBCenterInterceptInObject;
	...
	//_lengthOfBBCenterInterceptInObject;
	_lengthOfSTCenterInterceptInObject=lengthOfSTCenterInterceptInObject;
	//_lengthOfOriginalBBCenterInterceptInObject;
...
void printUsage() {
	std::string usage;
	usage.append("\n\nWheelNavCore-Test requires 25 parameters.");
	usage.append("\n\n\tUsage:");
	usage.append("\n\t1 : Request ID");
	usage.append("\n\t2 : HIT ID");
	usage.append("\n\t3 : Assignment ID");
...
void CallBackFunc(int event, int x, int y, int flags, void* userdata)
{
	...
	if(pointsCount==4) {
     ...
     if  ( event == cv::EVENT_LBUTTONDOWN ) {
          printf("\nLeft button of the mouse is clicked - position ( %d , %d)", x, y);
		  click = 1;
	 }
     else if  ( event == cv::EVENT_RBUTTONDOWN ) {
		 click = 1;
          printf("\nRight button of the mouse is clicked - position (%d , %d)", x, y);
     }
     else if  ( event == cv::EVENT_MBUTTONDOWN ) {
		 click = 1;
          printf("\nMiddle button of the mouse is clicked - position (%d , %d)", x, y);
     }
...
void drawSidewalTileRectOnImage()
{
	CvPoint point1 = cvPoint(tilePoint1_x, tilePoint1_y);
	CvPoint point2 = cvPoint(tilePoint2_x, tilePoint2_y);
	CvPoint point3 = cvPoint(tilePoint3_x, tilePoint3_y);
	CvPoint point4 = cvPoint(tilePoint4_x, tilePoint4_y);
	
	//IplImage* tempImage = originalImage;
	IplImage* tempImage = cvLoadImage(inputImageFile.c_str());
	cvLine(tempImage, point1, point2, cvScalar(255,0,0), 5, 8, 0);
	cvLine(tempImage, point2, point3, cvScalar(255,0,0), 5, 8, 0);
	cvLine(tempImage, point3, point4, cvScalar(255,0,0), 5, 8, 0);
	cvLine(tempImage, point4, point1, cvScalar(255,0,0), 5, 8, 0);

	cv::Mat tempMat = tempImage;
	cv::resize(tempMat, tempMat, cv::Size( tempMat.size().width, tempMat.size().height));
	IplImage *image = new IplImage(tempMat);
	
	cvShowImage("WheelNavCore-Test", image);
	cvWaitKey();
}

void acceptSidewalkTileDimensions() 
{
	int length=0, height=0;
	printf("\n\n\t\tEnter sidewalk tile length : ");
	cin >> sidewalkTileLength;
	printf("\n\t\tEnter sidewalk tile width : ");
	cin >> sidewalkTileWidth;
}

void printSidewalkTileCoOrdinates()
{
	int i;
	printf("\n\nSidewalk Tile Object Coordinates:\n");
	for(i=0;i<4;i++) {
		printf("\n\t Point %d :(%f, %f)", i+1, sidewalkTileObjectPoints[i].x, sidewalkTileObjectPoints[i].y);
	}

	printf("\n\nSidewalk Tile Image Coordinates:\n");
	for(i=0;i<4;i++) {
		printf("\n\t Point %d :(%f, %f)", i+1, sidewalkTileImagePoints[i].x, sidewalkTileImagePoints[i].y);
	}
}


void wheelnavCoreTest() {

	cout << "\n\n\t\tMeasured sidewalk tile length: " << measuredSidewalkTileLength;
	cout << "\n\t\tMeasured sidewalk tile width: " << measuredSidewalkTileWidth;

	cv::Mat tempMat = originalImage;
	cv::resize(tempMat, tempMat, cv::Size( tempMat.size().width, tempMat.size().height));
	IplImage *tempImage = new IplImage(tempMat);
	cvShowImage("WheelNavCore-Test", tempImage);
		
	cvSetMouseCallback("WheelNavCore-Test", CallBackFunc, NULL);
	cvWaitKey();
		
	while(pointsCount<4) {
	}

	drawSidewalTileRectOnImage();
	//acceptSidewalkTileDimensions();
	sidewalkTileLength=measuredSidewalkTileLength;
	sidewalkTileWidth=measuredSidewalkTileWidth;
}

int main(int argc, char* argv[]) {

	//WheelNavCore-Test : total 25 input parameters
